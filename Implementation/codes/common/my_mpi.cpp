...
{
    MPI_Recv(&MyMPI::buf_double, 100, MPI_DOUBLE, src, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void MyMPI::send_double(int dest, int tag, double d)
{
    ...
    MPI_Send(&MyMPI::buf_double, 100, MPI_DOUBLE, dest, tag, MPI_COMM_WORLD);
}

char MyMPI::recv_char(int src, int tag)
{
    MPI_Recv(&MyMPI::buf_char, 100, MPI_CHAR, src, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
{
    MyMPI::buf_char[0] = c;
    MPI_Send(&MyMPI::buf_char, 100, MPI_CHAR, dest, tag, MPI_COMM_WORLD);
}
