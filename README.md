...
The version is the development version I tested and ran on my computer.
...
```sh
mpic++ -n 4 run -d C  #for caltech, 2 groups with 2 slaves and 1 master per group
# or
mpic++ -n 2 run -d O  # for olivetti, 1 group with 1 slaves and 1 master
```
